...
const RencanaKerja = () => {
  ...
  useEffect(() => {
    const fetchWorkplans = async () => {
      ...
      console.log("Fetched workplans:", data);
      setWorkplanList(data);
    };
  ...
  return (
    <div className="relative mt-32 mb-16 p-6 m-7 lg:mx-44 max-w-4xl bg-white shadow-lg rounded-lg shadow-[0_-10px_20px_rgba(0,0,0,0),0_10px_30px_rgba(0,0,0,0.3)]">
      <FadeInSection>
        ...
        <p className="-mt-5 text-center text-sm sm:text-base">Daftar rencana kerja terbaru.</p>
      </FadeInSection>
        ...
        {workplanList.map((workplan) => (
          <FadeInSection key={workplan.id}>
            <div
              ...
              onClick={() => navigate(`/workplan/${workplan.id}`)}
            >
              <img
                src={
                  workplan.thumbnail
                    ? supabase.storage.from("media-uploads").getPublicUrl(workplan.thumbnail)?.data?.publicUrl || defaultImage
                    : defaultImage
                }
                alt="Thumbnail"
              ...
              <div className="p-3 flex-grow">
                <h4 className="text-base sm:text-lg font-bold text-white mt-2 mb-1 uppercase">
                  {truncateText(workplan.title, 20)}
                </h4>
                <p className="text-xs sm:text-sm text-gray-300">{new Date(workplan.created_at).toLocaleDateString()}</p>
              </div>
            </div>
          </FadeInSection>
        ))}
        {workplanList.length === 0 && (
          <p className="text-gray-500 text-center w-full">Tidak ada rencana kerja untuk ditampilkan.</p>
        )}
      </div>
    </div>
  );
